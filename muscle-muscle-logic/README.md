...
영향 없음) → 퍼즐을 완성하면 CLEAR — 마지막 운동 부위에 맞춰 포즈를 잡고, 최종
점수·최대 콤보·시간 보너스·근육 레벨을 보여준다 → 시간이 다 되면 대신 캐릭터가
넘어지며 GAME OVER → 두 결과 화면 모두 "다시 하기"로 완전히 초기화된 새 판을
시작한다.
...
강한 EXERCISE_HYPE 연출(스케일 팝 + 글로우)로 바뀐다.
...
- 캐릭터·아이템·연출은 전부 이모지/CSS placeholder다. 실제 스프라이트로 교체할 때는
  `CHARACTER_SPRITES`/`EXERCISE_DATA`/`CLEAR_POSES`/아이템 아이콘 마크업만 바꾸면
  되도록 설계했지만, 그림 자체는 아직 없다.
- 퍼즐은 3개(5×5 ×2, 10×10 ×1)뿐이며 스테이지 진행이나 퍼즐 로테이션은 없다 — 매
  판 항상 A(다이아몬드)로 시작한다.
