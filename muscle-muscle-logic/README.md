...
실행된다. 빌드 도구도, 서버도 필요 없다. 주인공 캐릭터는 사용자가 제공한 4단계
근육량 레퍼런스 일러스트(대기/스쿼트/이두 컬/오버헤드 프레스 × 4단계)를 크롭해
데이터 URI로 파일 안에 그대로 내장했다 — 외부 이미지 파일이나 네트워크 요청은
전혀 없다.
...
영향 없음) → 퍼즐을 완성하면 CLEAR — 마지막으로 트레이닝한 운동 포즈 그대로,
그 시점 근육 레벨의 실제 일러스트로 캐릭터가 등장하고 최종 점수·최대 콤보·시간
보너스·근육 레벨을 보여준다 → 시간이 다 되면 대신 캐릭터가 넘어지며 GAME OVER →
두 결과 화면 모두 "다시 하기"로 완전히 초기화된 새 판을 시작하는데, 이때 직전
판과 같은 퍼즐이 곧바로 반복되지 않도록 다음 퍼즐이 나머지 중에서 무작위로
뽑힌다. 캐릭터는 CLEAR 순간에만 바뀌는 게 아니라 점수가 오를 때마다(HUD의
"근육 LV." 숫자와 동일한 기준으로) PLAYING 화면에서도 실시간으로 더 근육질
일러스트로 바뀐다. 정답/오답/콤보/아이템 사용/CLEAR/GAME OVER 각각에 짧은
WebAudio 효과음이 붙어 있다.
...
위 폴리시 패스 직후, 사용자가 4단계 근육량(마름/일반인/다져진 체형/운동의 신) ×
(대기 포즈 + 스쿼트·이두 컬·오버헤드 프레스) 레퍼런스 일러스트를 제공해 이모지
placeholder를 실제 캐릭터로 교체했다.

- 레퍼런스 시트 2장(운동 포즈 3종×4단계 그리드, 대기 포즈 4단계 일렬)에서 캡션·
  화살표·번호·다른 패널이 전혀 섞여 들어가지 않도록 16장을 각각 크롭하고, 표시
  크기(최대 높이 약 140px)에 맞춰 WebP로 재인코딩해 전체 약 200KB로 내장했다.
- `CHAR_ART[pose][level]` 형태의 조회 테이블로 관리하며, 레퍼런스에 없는
  레벨 5는 레벨 4 이미지를 그대로 재사용한다(데이터를 중복 저장하지 않고 참조만
  공유). CLEAR 화면에서 레벨 4와 5를 구분하는 건 기존에 있던
  `muscle-lv-5`의 CSS 확대/글로우다.
- IDLE/EXERCISE/EXERCISE_HYPE/CLEAR는 각각 맞는 포즈+레벨 이미지를 보여준다.
  MISTAKE와 GAME OVER는 별도 "놀란/쓰러진" 원화가 없으므로 대기 포즈 이미지를
  재사용하고, 기존의 흔들림/회전 CSS 애니메이션으로 상태를 표현한다(애니메이션은
  이모지든 이미지든 동일하게 동작한다).
- CLEAR 화면의 운동부위 배지(이모지)는 제거했다 — 이제 캐릭터가 실제로 그
  운동을 하는 그림 자체가 보이므로 별도 배지가 오히려 중복이자 스타일 불일치
  요소였다.
- 캐릭터 하나에 다른 인물이 함께 나오는 프레임, 레퍼런스에 있던 캡션·화살표·
  번호가 노출되는 크롭은 없는지 직접 눈으로 16장을 전수 검토했다.
...
| `CHAR_ART` / `getCharArt` | 사용자 제공 레퍼런스에서 크롭한 캐릭터 이미지(대기 + 운동 3종 × 근육 레벨 1~4, WebP data URI로 내장, 레벨 5는 레벨 4를 재사용)와 `getCharArt(pose, level)` 조회 함수 |
...
| `CharacterManager` | 캐릭터 상태 머신(IDLE/EXERCISE/EXERCISE_HYPE/MISTAKE/CLEAR/GAME_OVER). `MuscleManager.getLevel()`로 그 순간의 근육 레벨을 읽어 `getCharArt(pose, level)`로 실제 이미지를 고르고(운동 중엔 현재 퍼즐의 `exercise`, 그 외엔 대기 포즈), CLEAR/GAME_OVER 전환 시에는 (그 순간 화면에 실제로 보이는) 결과 화면 자체의 스프라이트(`#clearSprite`/`#gameOverSprite`)에 포즈·낙상 애니메이션을 적용한다 |
...
정답을 맞히면 해당 운동 포즈(현재 근육 레벨의 실제 일러스트)가
`CONFIG.EXERCISE.DURATION_MS`(기본 0.6초)만큼 재생된다. 이때 콤보가
`CONFIG.COMBO.HYPE_THRESHOLD`(기본 5) 이상이면 더 빠르고 강한 EXERCISE_HYPE
연출(스케일 팝 + 글로우)로 바뀐다. CLEAR 화면에서는 캐릭터가 마지막으로 트레이닝한
운동을 그 최종 근육 레벨로 다시 한번 보여준다.
...
캐릭터 이미지 자체가 레벨 1~4에서 이미 눈에 띄게 달라지고(레퍼런스 원본 4단계),
레벨 5는 그림은 레벨 4와 같지만 CLEAR 화면의 `muscle-lv-1`~`muscle-lv-5` CSS
클래스가 추가로 크기/글로우를 키워 레벨 4와 구분되게 한다.
...
- 주인공 캐릭터는 실제 레퍼런스 일러스트로 교체됐지만, MISTAKE·GAME OVER 전용
  원화(예: 놀란 표정, 쓰러진 포즈)는 레퍼런스에 없어서 대기 포즈 이미지를
  재사용하고 CSS 애니메이션(흔들림/회전)만으로 그 상태를 표현한다. 아이템
  아이콘(🏋️/🥤/🧑‍🏫)과 덤벨 낙하 소품, HUD 아이콘은 여전히 이모지다 — "주인공
  캐릭터"라는 요청 범위에 맞춰 캐릭터 본체만 교체했다.
